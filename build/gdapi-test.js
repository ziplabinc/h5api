/*
 * build date : 2018-06-13 04:48:47
 */
window.gdApi = window.gdApi || {};

gdApi.isMobile = function () {
    var check = false;
    (function (a) {
...
gdApi.Point = new function() {
    ...
    this.init = function() {
        if(document.body === null || document.body === undefined) {
            console.error("gdApi.Point.init must be call after window.onload");
            return;
        }
        ...
        this._styleDOM.innerText = styleText.join(" ");
        for(var key in this._imageURI) {
            this._styleDOM.innerText = this._styleDOM.innerText.replace("["+key+"]", this._imageURI[key]);
        }        
        document.body.appendChild(this._styleDOM);
    ...
    this._openPopup = function(data) { // console.log(data);
        ...
        if(data.status == "not_login") {
            ...
            this.DOM.mainPopup.style.backgroundImage = "url("+this._imageURI.bgNotlogin+")";
        }
            ...
            this.DOM.mainPopup.style.backgroundImage = "url("+this._imageURI.bgSuccess+")";
        }
    ...
    this.call = function(opt) {
        if(opt === undefined)                       opt = {}
        if (opt.env === undefined)                  console.error("env arg is undefined");
        ...
        xhr.onreadystatechange = function (e) {
            if (xhr.readyState === 4) { // DONE
                if (xhr.status === 200) { // OK
                    ...
                    console.error('Error: ' + xhr.status+" "+xhr.statusText+" ("+xhr.responseURL+")");
                }
            }
        }.bind(this);
    }.bind(this)
};
gdApi.VERSION = "Testing on 1.1.0"